...
/**
 * Formulario de edición de un portafolio, compartido entre el autoservicio
 * del dueño (/aliados/estado/[token], vía actualizarPortafolio) y la edición
 * del moderador (/admin/aliados, vía editarPortafolioModerador). `accion` es
 * la server action ya bindeada con su identificador (token o id) — este
 * componente no sabe ni le importa cuál de los dos es.
 */
export function FormularioEdicionPortafolio({
  portafolio,
  categorias,
  accion,
}: {
  ...
  // Prefijo por instancia: en /admin/aliados se puede abrir la edición de
  // dos fichas a la vez, y sin esto los ids de sus campos (id="direccion",
  // etc.) se duplican — el segundo <label htmlFor> apunta al campo de la
  // primera ficha en vez del suyo.
  const idPrefijo = useId();
  const idCampo = (nombre: string) => `${idPrefijo}-${nombre}`;
  ...
  return (
    <form action={accionFormulario} className="mt-10 flex flex-col gap-12">
      ...
      >
        ...
        <div className="grid max-w-xl grid-cols-1 gap-6">
          <CampoFormulario id={idCampo('direccion')} etiqueta="Dirección" requerido errores={err('direccion')}>
            {(p) => (
              <input
                {...p}
                name="direccion"
          ...
          <CampoFormulario id={idCampo('barrio')} etiqueta="Barrio" requerido errores={err('barrio')}>
            {(p) => (
              <SelectConOtro
                {...p}
                name="barrio"
      ...
      <Seccion numero="02" titulo="Tu negocio">
        <CampoFormulario id={idCampo('nombre')} etiqueta="Nombre del negocio" requerido errores={err('nombre')}>
          {(p) => (
            <input
              {...p}
              name="nombre"
        ...
        {categoriaId === 'otros' && (
          <CampoFormulario id={idCampo('categoria_otra')} etiqueta="¿Qué tipo de negocio es?" requerido errores={err('categoria_otra')}>
            {(p) => (
              <input
                {...p}
                name="categoria_otra"
      ...
      >
        <div className="flex flex-col gap-6">
          <CampoFormulario id={idCampo('whatsapp')} etiqueta="WhatsApp" requerido errores={err('whatsapp')}>
            {(p) => (
              <input
                {...p}
                name="whatsapp"
          ...
          <CampoFormulario id={idCampo('correo')} etiqueta="Correo" errores={err('correo')}>
            {(p) => (
              <input
                {...p}
                name="correo"
      ...
      >
        <CampoFormulario id={idCampo('horario')} etiqueta="¿Cuándo atiendes?">
          {(p) => (
            <ChipsMultiple
              {...p}
              name="horario"
        ...
        <CampoFormulario id={idCampo('medios_pago')} etiqueta="¿Cómo te pagan?">
          {(p) => (
            <ChipsMultiple
              {...p}
              name="medios_pago"
