...
export function TarjetaPaso({ paso }: { paso: PasoFormalizacion }) {
  ...
  return (
    <li className="flex h-full flex-col border border-tinta/12 p-5 transition-colors hover:border-azul">
      ...
      <p className="mt-0.5 font-sans text-xs text-tinta/60">{paso.entidad}</p>
      ...
      <div className="mt-3">
        {requisitosLargos ? (
          ...
          <>
            <p className="font-sans text-xs text-tinta/60">Necesitas tener:</p>
      ...
      <div className="mt-auto pt-4">
        <a
          ...
          className="inline-flex items-center gap-1.5 border border-azul-texto/40 px-3 py-1.5 font-sans text-xs text-azul-texto transition-colors hover:border-azul-texto"
        >
          Ver en la página oficial
          <span aria-hidden="true">↗</span>
        </a>
        <p className="mt-2 font-sans text-xs text-tinta/60">
